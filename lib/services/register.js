...
ServicesRegister.prototype._bindToEvents = function(providerId, provider) {
  ...
  function emitOverEvents(data) {
    var eventId = this.event;
    logger.debug(eventId);
    self.events.emit(eventId, data);
    logger.debug(providerId + '.' + eventId);
    self.events.emit(providerId + '.' + eventId, data);
  }

  try {
    provider.on('*', emitOverEvents);
    provider.on('*.*', emitOverEvents);
  } catch(err) {
